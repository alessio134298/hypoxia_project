...
# H2BK120ac
path_H2BK120ac = "/media/alessio/Data/hypoxia/ChIPseq/ROSE_analysis/rose_bitbucket_python2_results/RESULTS_ROSE_H2BK120ac"
dir_H2BK120ac = c("C1B", "C1E", "C1H",
         "C2B", "C2E", "C2H",
         "K125B1", "K125E1", "K125H1",
         "K26B1", "K26E1", "K26H1")
filename = "_peaks_AllEnhancers.table.txt"

Enhancers.H2BK120ac <- list()

for (i in dir_H2BK120ac) {
  tmp.path = file.path(path_H2BK120ac,i,paste0(i,filename))
  tmp.df <- read.delim(tmp.path, skip = 5, header = T) |> 
    dplyr::mutate(Signal = human_only_sorted.bam - input_human_only_sorted.bam) |> 
    dplyr::mutate(Signal = ifelse(Signal > 0, Signal, 0)) |> 
    dplyr::arrange(Signal)
  Enhancers.H2BK120ac[[i]] <- tmp.df
}
```
...
SEs.plot <- function(df, name) {
  ...
  TEs_count <- sum(df$isSuper == 0)
  SEs_count <- sum(df$isSuper == 1)
...
SEs.plots.H2BK120ac <- lapply(names(Enhancers.H2BK120ac), function(name) {
  plot <- SEs.plot(Enhancers.H2BK120ac[[name]], name)
})

SEs_plot_grid_normoxia <- gridExtra::grid.arrange(grobs = SEs.plots[c(1,4,7,10)], ncol = 2, top=grid::textGrob("H3K27ac Normoxia Enhancers"))
SEs_plot_H2BK120ac_grid_normoxia <- gridExtra::grid.arrange(grobs = SEs.plots.H2BK120ac[c(1,4,7,10)], ncol = 2, top=grid::textGrob("H2BK120ac Normoxia Enhancers"))

ggsave(SEs_plot_grid_normoxia, filename = "/media/alessio/Data/hypoxia/ChIPseq/plot/Enhancers_H3K27ac_normoxia_ROSE.pdf", height = 6, width = 10)
ggsave(SEs_plot_H2BK120ac_grid_normoxia, filename = "/media/alessio/Data/hypoxia/ChIPseq/plot/Enhancers_H2BK120ac_normoxia_ROSE.pdf", height = 6, width = 10)
```
